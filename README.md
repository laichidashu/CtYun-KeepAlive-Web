...
在此基础上，项目已整合为一套**一体化 Web 管理平台**：除多账号保活外，还提供定时任务（AI 对话 / 云电脑挂机，驱动 Python 脚本）、**平台任务完成情况面板**（一键查看/补做各账号未完成任务）、积分自动兑换、**自动更新**、运行环境自检、实时日志推送与图形化控制台。
...
| **平台任务完成情况** | 按账号列出今日各任务的完成状态（已完成 / 未执行 / 失败 / 运行中），支持**全部 / 未完成 / 已完成**筛选，一键补做未完成的 AI 对话，可勾选「检测到未完成时自动补做」 |
| **兑换** | 兑换配置读写、奖励列表拉取、兑换计划判定、立即执行兑换 |
| **日志** | 实时日志终端（SSE 推送），按级别着色 |
| **设置** | 保活周期、各类超时、Python 路径、脚本目录、互斥模式、轮询间隔等 |

### 3. 平台任务完成情况与一键补做
- 📋 **按账号汇总**：今日每个账号的每条任务一行，显示今日状态徽章（今日已完成 / 今日未执行 / 今日失败 ×N / 运行中 / 已停用）、今日最近一次执行摘要、上次/下次运行时间。
- 🔍 **完成判定双重口径**：任务历史记录 + **平台任务预检**——执行前先调平台接口核对「对话」「使用1小时」等平台任务是否已完成，已完成则直接记「已完成，跳过」，不重复跑浏览器（fail-open：预检失败照常执行）。
- ⚡ **串行补做队列**：一键补做/自动补做会将所有缺失任务放入**串行队列**逐个执行（每个任务执行前二次确认仍缺失），彻底避免并发触发被「浏览器互斥」瞬间拒绝的问题。
- 🏷️ **状态筛选**：面板顶部实时统计「已完成 X · 未完成 Y · 运行中 Z」，可按 全部 / 未完成 / 已完成 过滤。

### 4. 自动更新
- 🚀 **启动时自动检查**：`start.ps1` 在环境自检后调用 `backend/updater.py --update`，从 GitHub 拉取最新代码。
- 🌐 **网页手动更新**：设置面板提供「检查更新 / 立即更新」按钮（`/api/update/check`、`/api/update/run`），更新完成后服务自动重启。
- 🛡️ **数据安全**：更新只覆盖代码文件，`accounts.json` / `jobs.json` / `redeem_config.json` / `devices/` / `logs/` / `.venv/` / Cookie 等运行数据全部保留。
...
- 🛜 **断连自愈（Errno 10054）**：平台在云电脑关机/重置时会强制断开保活连接（10054）。引擎识别该特征后自动探测真实状态、补发开机指令并等待 60s 重试，**不计入失败次数、不误重建会话**——彻底消灭日志刷屏式 10054 错误。
- 🧩 **验证码本地 OCR**：ddddocr + 灰度/多阈值二值化/4× 放大管线（合规率 12/12），外部 OCR 接口仅作兜底，不受代理波动影响；登录失败自适应重试（每次重新取验证码）。
...
> 启动逻辑全部在 `start.ps1`（PowerShell 脚本，不受批处理换行符影响）；`启动服务.bat` 只是它的 3 行启动壳。全部流程写入 `startup.log`，闪退/报错时先看它。

启动脚本会自动完成以下步骤，全部就绪后才开始运行服务：
1. **定位解释器**：项目内置 `.venv` → 系统 `python` → `py` 启动器 → 常见安装路径；**都找不到时自动静默安装 Python 3.12.8**（华为云镜像优先下载，per-user 安装并自动加入 PATH）
2. **创建虚拟环境**：无 `.venv` 时自动创建，依赖隔离不污染系统 Python
3. **环境自检 + 自动装依赖**：运行 `backend/bootstrap.py` 检查 `requests / pillow / DrissionPage / ddddocr`，缺失项自动 `pip` 安装（**清华镜像 → 阿里云 → 官方源**依次尝试），安装复检通过才继续
4. **自动更新**：调用 `backend/updater.py --update` 从 GitHub 拉取最新代码（保留全部运行数据），无网络时自动跳过
5. **防火墙（尽力而为）**：自动添加入站 TCP 8080 放行规则，局域网其他设备即可访问；失败时提示手动放行
6. **启动服务**：运行 `backend/server.py`，控制台打印局域网访问地址
...
所有数据都落在数据目录（本地默认为工程根目录，可用 `CTYUN_DATA_DIR` 指定）：
...
│   ├── keepalive.py       # 保活引擎（会话状态机/退避/心跳/断连自愈/自动开机）+ 重启调度
...
│   ├── jobs.py            # 定时任务服务 + cron 调度器 + 平台任务汇总/串行补做
│   ├── cronx.py           # 5 段 cron 解析器
│   ├── scriptrunner.py    # Python 任务脚本运行器（超时/进程树回收）
│   ├── redeem.py          # 积分兑换（通道 A）+ 调度策略 + 平台任务预检
│   ├── bootstrap.py       # 环境自检 + 依赖自动安装（清华→阿里云→官方镜像依次尝试）
│   ├── updater.py         # 自动更新（GitHub 拉取 + 数据保护 + 服务重启）
│   ├── envprobe.py        # 环境自检
...
├── requirements.txt       # 任务脚本依赖（requests / pillow / DrissionPage / ddddocr）
├── accounts.json          # 运行时数据：账号 + 全部配置（首次启动自动生成，已 gitignore）
├── devices/               # 运行时数据：设备码（已 gitignore）
├── 启动服务.bat            # Windows 一键启动（3 行壳，实际逻辑在 start.ps1）
├── start.ps1              # 启动主脚本：装 Python → venv → 自检装依赖 → 自动更新 → 防火墙 → 服务
├── startup.log            # 启动过程日志（排查启动问题先看这里）
├── .gitattributes         # 行尾策略（*.bat/*.ps1 按字节存储，防止 CRLF/LF 引发闪退）
