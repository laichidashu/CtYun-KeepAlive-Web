...
- 🌐 **全部平台任务统计**：点「🌐 拉取全部平台任务」，后端串行登录全部账号拉取平台积分任务（登录较慢，走后台任务 + 前端轮询，不阻塞页面）。面板顶部汇总「平台任务：已完成 X · 进行中 Y · 未完成 Z」，每个账号名下方直接显示「平台任务 3 项：✓1 · 进行中 1 · 未完成 1」，点「平台任务 ▾」还能展开逐项进度（如 `使用1小时 2100/3600`）。
- 🎯 **目标值智能推断**：平台接口只返回进度、不给目标值，因此从任务名反推目标——「使用1小时」=3600（进度单位为秒）、「与AI对话1次」=1、「登录AI云电脑」=1，从而准确区分 已完成 / 进行中 / 未完成。
...
- 🌐 **网页手动更新**：「设置 → 版本与更新」面板显示本地版本 / 远端版本 / 提交时间 / 提交说明，提供 **🔍 检查更新** 与 **⬆️ 更新到最新版并重启** 两个按钮（`/api/update/check`、`/api/update/run`）。进入设置页会自动静默检查一次；点击更新后下载覆盖代码、自动重启服务，页面检测到服务恢复会自动刷新。
